...
class newListing extends user{
  //Αυτή η κλάση συνδέεται με την δημιουργία μίας νέας αγγελίας καλείτε δηλαδή όταν ο χρήστης πατήσει create listing 
    //Πίνακας με όλα τα δειγματικά listings για τον έλεγχο ορθής λειτουργίας του κώδικα πριν την σύνδεση με βάση δεδομένων. Τα αντικείμενα
    //σε αυτήν την φάση έχουν μία εικόνα, τίτλο, περιγραφή, το αντικείμενο με το οποίο ο χρήστης θα ήθελε να ανταλλάξει το δικό του, πεδίο free
    //το οποίο θα είναι true σε περίπτωση που θέλει να δωρίσει το αντικείμενο του και τέλος το id του χρήστη που έκανε την ανάρτηση.
    static listings=[
  ...
  //static Listing=require('./models/listing.js')
    constructor(params){
        super(params);
        this.id=params.id;
   ...
   //Αυτή η συνάρτηση επιστρέφει ένα listing με βάση την εικόνα εκτός αν περαστεί σαν παράμετρος η κενή συμβολοσείρα όπου τότε επιστρέφει
   //τον πίνακα με όλες τις αγγελίες που έχουν αναρτηθεί. Μέχρι να γίνει σύνδεση με βάση δεν υπάρχει μοναδικό id γιαυτό προσορινά χρησιμοποιείται
   //το url της εικόνας.
  ...
  //Η συνάρτηση που υπάρχει στις κλάσει που αναπαριστούν view και επιστρέφει τον κώδικα html που θα προστεθεί στο body της αρχικής σελίδας.
  //Ουσιαστικά δημιουργείτε μία φόρμα στην οποία ο χρήστης παραθέτει μία εικόνα, τίτλο αγγελίας, περιγραφή, το αντικείμενο με το οποίο θα
  //ήθελε να ανταλλάξει το δικό του και ένα checkbox το οποίο τσεκάρει εφόσον θέλει να δωρίσει το αντικείμενο του.
  async getElement() {
    if (
...
//Αυτή η μέθοδος αντλεί τα δεδομένα από την html φόρμα για την δημιουργία νέας αγγελίας.
 async extract() {
  ...
  //Το κουμπί submit της φόρμας όπου λαμβάνει τα δεδομένα από τα πεδία της φόρμας και εφόσον έχουν συμπληρωθεί όλα προσθέτει την αγγελία
  //στον πίνακα που περιέχει όλες τις αγγελίες και ενημερώνει τον χρήστη για την επιτυχή καταχώρηση. Τυπώνει μήνυμα λάθους σε περίπτωση
  //που κάποιο πεδίο έχει παραμείνει άδειο.
  forms.children[5].addEventListener("click", async (event) => {
    ...
    if(this.validate(details)){
      ...
      let ans=await axios.post('/newListing',details);
      if(ans.data){
           this.alertUser("Listing added succesfully","success")
      }else{
        console.log("didnt reply with true DB PROBLEM");
      }
    }else{
